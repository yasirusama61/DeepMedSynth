...
SAVE_DIR = "/kaggle/working/segmentation_results_v3"
...
class SliceDataset(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        img_vol = np.load(self.image_paths[vol_idx])   # shape (3,128,128,128)
        mask_vol = np.load(self.mask_paths[vol_idx])   # shape (128,128,128)

        for i in range(self.batch_size):
            s_idx = (slice_idx + i) % 128
            img_slice = img_vol[:, :, :, s_idx].transpose(1, 2, 0)  # (128,128,3)
            mask_slice = mask_vol[:, :, s_idx][..., np.newaxis]      # (128,128,1)
...
def build_unet(input_shape=(128,128,3)):
    ...
    u1 = layers.UpSampling2D()(c3); u1 = layers.concatenate([u1,c2]); c4 = conv_block(u1, 64)
    u2 = layers.UpSampling2D()(c4); u2 = layers.concatenate([u2,c1]); c5 = conv_block(u2, 32)
